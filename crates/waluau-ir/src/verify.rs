            ...
            match instruction {
                ...
                Instruction::GrowableArrayNew {
                    ...
                    for &element in initial_elements {
                        ...
                        if element_value_ty != *element_ty {
                            return Err(Diagnostic::new(format!(
                                "growable array element in block {:?} expects {}, got {}",
                                block.id, element_ty, element_value_ty
                            )));
                        }
                    }
                }
                Instruction::GrowableArrayGet {
                    array,
                    index,
                    element_ty,
                } => {
                    ...
                    if array_ty != expected_array_ty {
                        return Err(Diagnostic::new(format!(
                            "growable array get in block {:?} expects {}, got {}",
                            block.id, expected_array_ty, array_ty
                        )));
                    }
                    let index_ty = require_dominating_definition(
                    ...
                    if index_ty != Type::Numeric(NumericType::I32) {
                        return Err(Diagnostic::new(format!(
                            "growable array get index in block {:?} must be i32",
                            block.id
                        )));
                    }
                }
                Instruction::GrowableArraySet {
                    array,
                    index,
                    value,
                    element_ty,
                    ...
                    if array_ty != expected_array_ty {
                        return Err(Diagnostic::new(format!(
                            "growable array set in block {:?} expects {}, got {}",
                            block.id, expected_array_ty, array_ty
                        )));
                    }
                    let index_ty = require_dominating_definition(
                    ...
                    if index_ty != Type::Numeric(NumericType::I32) {
                        return Err(Diagnostic::new(format!(
                            "growable array set index in block {:?} must be i32",
                            block.id
                        )));
                    }
                    let value_ty = require_dominating_definition(
                    ...
                    if value_ty != *element_ty {
                        return Err(Diagnostic::new(format!(
                            "growable array set value in block {:?} expects {}, got {}",
                            block.id, element_ty, value_ty
                        )));
                    }
                }
                Instruction::GrowableArrayLen { array, element_ty } => {
                    let array_ty = require_dominating_definition(
                        &definitions,
                        &dominators,
                        &seen_in_block,
                    ...
                    if array_ty != expected_array_ty {
                        return Err(Diagnostic::new(format!(
                            "growable array len operand in block {:?} expects {}, got {}",
                            block.id, expected_array_ty, array_ty
                        )));
                    }
                }
                Instruction::GrowableArrayPush {
                    array,
                    value,
                    element_ty,
                } => {
                    ...
                    if array_ty != expected_array_ty {
                        return Err(Diagnostic::new(format!(
                            "growable array push in block {:?} expects {}, got {}",
                            block.id, expected_array_ty, array_ty
                        )));
                    }
                    let value_ty = require_dominating_definition(
                    ...
                    if value_ty != *element_ty {
                        return Err(Diagnostic::new(format!(
                            "growable array push value in block {:?} expects {}, got {}",
                            block.id, element_ty, value_ty
                        )));
                    }
                }
...
fn infer_instruction_type(
    ...
    match instruction {
        ...
        Instruction::GrowableArrayNew { element_ty, .. } => Ok(Type::Array(Box::new(element_ty.clone()))),
        Instruction::GrowableArrayGet { element_ty, .. } => Ok(element_ty.clone()),
        Instruction::GrowableArraySet { .. } => Ok(Type::Numeric(NumericType::I32)),
        Instruction::GrowableArrayLen { .. } => Ok(Type::Numeric(NumericType::I32)),
        Instruction::GrowableArrayPush { .. } => Ok(Type::Unit),
