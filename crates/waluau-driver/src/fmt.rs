    ...
    I: IntoIterator<Item = OsString>,
{
    ...
    for path in &options.paths {
        collect_walu_files(path, &mut files).map_err(|e| vec![e])?;
    }
    files.sort();
    files.dedup();
...
/// directory.
fn collect_walu_files(path: &Path, out: &mut Vec<PathBuf>) -> Result<(), Diagnostic> {
    let meta =
    ...
    if meta.is_dir() {
        ...
        for entry in entries {
            ...
            collect_walu_files(&entry.path(), out)?;
        }
