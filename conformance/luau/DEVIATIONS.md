...
At commit `d2ab33c3` (after PR #647), the directory has **1,065 chunks**:
**355 enabled** and **710 pending**. These numbers are a snapshot, not a target.
...
| Typed math-library completion | `waluau-q7qg.11` | Remaining `math.*` chunks, split from coercion-only checks |
