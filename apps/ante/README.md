...
A vault is one heist inside a longer roguelike run of nine. The pool is the
run's, not the vault's: clearing one carries its remaining mana into the next,
and every cast spends from it for good. Between two vaults the run stops at the
shop, where that same mana buys spell scrolls and healing potions.
...
through the fence and into later vaults, and clearing one does not repair them.
The shop is the only place they move upward, one potion at a time. Every loss
therefore shrinks the run's remaining margin for error, and losing the last
heart ends the run as soon as that hand resolves; a potion cannot buy it back.
The next heist is then the first vault of a fresh run. R restarts the run from
vault one at any point, which is also why a settled vault is never left standing
on the board.
...
their familiar ranks across twelve dedicated folk-print court plates. Every
school has a book-reading apprentice, an orb-bearing seer, and a staff-wielding
mage: Red channels power and emotion, Blue trains meditative reciprocity, Black
reads medicine and vital energy, and Green practices druidic transformation.
The Ace is a single major omen. This keeps the deck mechanically legible
without borrowing the mirrored portraits or suit layout of a poker deck.
...
road route. Nodes 1 and 2 are the two market squares. A separately reserved churchyard has its own road access node. With a river and no bridges,
...
access point on a particular road edge. Market squares and the churchyard reserve space before houses are placed; their
centres, clearance radii, kinds, names, and access nodes are exposed in `landmarks`.
All road indices are 1-based; each edge
...
and frontage slots rather than subdividing entire blocks into parcel polygons. Relief is shown in tenths (10 means 1.0); it changes road connection
