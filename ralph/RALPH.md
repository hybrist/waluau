...
2. If `.beads/embeddeddolt/` is missing, run `pnpm install && pnpm exec bd bootstrap --yes`
3. Run `pnpm exec bd ready` to see available tasks
4. Run `pnpm exec bd list --status in_progress` to check for any work left mid-flight
...
2. Run `pnpm exec bd show <id>` to read the full task and acceptance criteria
3. Run `pnpm exec bd update <id> --status in_progress` to claim it

## On Finishing ONE Task

1. Run `./check` to confirm all quality gates pass
2. Close completed tasks: `pnpm exec bd close <id>`
